...
describe('projectsLoader', () => {
  ...
  describe('with search queries', () => {
    it('returns projects when search query is present', async () => {
      ...
      mockedProjectService.getAll.mockResolvedValue(mockProjects);

      const result = (await projectsLoader(createLoaderArgs('http://localhost?q=test'))) as ProjectsLoaderData;

      expect(mockedProjectService.getAll).toHaveBeenCalledWith('test');
    ...
    it('handles empty search query', async () => {
      ...
      expect(mockedProjectService.getAll).toHaveBeenCalledWith('');
      expect(result).toEqual({ projects: mockProjects });
    });
    ...
    it('handles special characters in query', async () => {
      ...
      expect(mockedProjectService.getAll).toHaveBeenCalledWith('react node');
      expect(result).toEqual({ projects: mockProjects });
    });
    ...
    it('handles URL with hash and query', async () => {
      ...
      expect(mockedProjectService.getAll).toHaveBeenCalledWith('search');
      expect(result).toEqual({ projects: mockProjects });
    });
  });
  ...
  describe('error handling', () => {
    it('throws when service fails', async () => {
      mockedProjectService.getAll.mockRejectedValue(new Error('Service error'));
